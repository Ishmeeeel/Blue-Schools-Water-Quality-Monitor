...
async def trigger_training(background_tasks: BackgroundTasks):
    """
    Trigger CPT parameter learning from accumulated field survey data.
    Runs in background — returns immediately with a job confirmation.
    Call this manually after importing CSV data, or let the
    Render Cron Job call it automatically every night.
    """
    from train import run_training
    background_tasks.add_task(run_training)
    return {
        "message":   "Training job started in background.",
        "check":     "Watch Render logs for progress.",
        "minimum":   "Needs 20+ surveys to update CPTs.",
...
async def training_status():
    """Check the latest training run result from Supabase."""
    import database as db
    if not db.DB_AVAILABLE:
        return {"status": "db_offline"}
    try:
        result = (
            db.supabase.table("cpt_store")
            .select("trained_at,n_samples,status,version")
            .eq("status", "active")
            .limit(1)
            .execute()
        )
        if result.data:
            return {"status": "trained", "latest": result.data[0]}
        return {"status": "never_trained", "message": "No CPTs in store yet"}
    except Exception as e:
